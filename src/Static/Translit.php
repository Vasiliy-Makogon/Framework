...
class Krugozor_Static_Translit
{
    public static function UrlTranslit($string, $allow_slashes = TR_NO_SLASHES)
    {
        ...
        $string = mb_strtr($string, $LettersFrom, $LettersTo);
        $string = strtr($string, $BiLetters);

        $string = preg_replace("/j{2,}/u", "j", $string);
        $string = preg_replace("/[^" . $slash . "0-9a-z_\-]+/u", "", $string);
    ...
    public static function Supertag($string, $allow_slashes = TR_NO_SLASHES)
    {
        ...
        $string = Krugozor_Static_Translit::UrlTranslit($string, $allow_slashes);
