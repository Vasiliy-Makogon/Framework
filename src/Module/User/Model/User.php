...
class Krugozor_Module_User_Model_User extends Krugozor_Model
{
    ...
    (
        ...
        'sex' => array(
            'type' => 'Krugozor_Module_User_Type_Sex',
            'db_element' => true,
    ...
    public function setData($data, array $excluded_keys = array())
    {
        ...
        if ($data['age_day'] && $data['age_month'] && $data['age_year']) {
            $age = Krugozor_Type_Datetime::createFromFormat(
                'j-n-Y H:i:s',
    ...
    public function getUniqueCookieId()
    {
        if (!$this->unique_cookie_id) {
            $this->setUniqueCookieId(Krugozor_Static_String::getUnique());
        }
    ...
    {
        if ($this->age && $this->age instanceof Krugozor_Type_Datetime) {
