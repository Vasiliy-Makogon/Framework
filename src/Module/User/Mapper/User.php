...
class Krugozor_Module_User_Mapper_User extends Krugozor_Mapper_Common
{
    /**
    ...
    public function getUsersListWithResidence($params = array())
    {
        ...
        $sql = 'SELECT SQL_CALC_FOUND_ROWS
                ...
                LEFT JOIN
                    `user-country`
                ON
                    `user`.`user_country` = `user-country`.`id`
                LEFT JOIN
                    `user-region`
                ON
                    `user`.`user_region` = `user-region`.`id`
                LEFT JOIN
                    `user-city`
                ON
                    `user`.`user_city` = `user-city`.`id`
                LEFT JOIN
                    `advert`
                ON
                    `user`.`id` = `advert`.`advert_id_user`' .
     ...
     *
     * @param Krugozor_Module_Group_Model_Group
     * @return bool
     */
    public function setDefaultGroupForUsersWithGroup(Krugozor_Module_Group_Model_Group $group)
    {
        $sql = 'UPDATE ?f SET
     ...
     *
     * @param void
     * @return Krugozor_Cover_Array Object
     */
