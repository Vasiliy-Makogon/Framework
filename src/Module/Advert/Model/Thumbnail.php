 ...
 *
 * Вызывается метод Krugozor\Framework\Module\Advert\Mapper\Thumbnail::unlink($thumbnail) - изображение отвязывается от
 * сущности в конкретном контроллере.
 * Далее в CRON-обработчике вызывается Krugozor\Framework\Module\Advert\Mapper\Thumbnail::getThumbnailsNotRelatedToAdverts(),
...
class Thumbnail extends Model
{
    ...
    public function getFullHttpPath()
    {
        ...
        if (!$this->full_http_path) {
            $directory_generator = new DirectoryGenerator($this->file_name);
    ...
    {
        $directory_generator = new DirectoryGenerator($this->file_name);
        $file_1 = $directory_generator->create(
            DOCUMENTROOT_PATH . Registry::getInstance()->UPLOAD['THUMBNAIL_150x100']
        ) . $this->file_name;
        $file_2 = $directory_generator->create(
            DOCUMENTROOT_PATH . Registry::getInstance()->UPLOAD['THUMBNAIL_800x800']
        ) . $this->file_name;
