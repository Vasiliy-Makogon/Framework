...
class BackendEdit extends BackendCommon
{
    ...
    public function run()
    {
        ...
        if (empty($this->advert)) {
            $this->advert = $this->getMapper('Advert/Advert')->createModel(
                $this->getMapper('User/User')->findModelById(-1)
            );
        } else {
    ...
    protected function post()
    {
        if (!$this->checkAccess()) {
            ...
            $url = $this->getRequest()->getCanonicalRequestUri()->getSimpleUriValue() . '?id=' . $this->advert->getId();
        ...
        $this->advert->setData($this->getRequest()->getPost('advert'));
        ...
        if (!$this->advert->getValidateErrorsByKey('id_user')) {
            $validator->add('id_user', new UserIdExists(
        ...
        if ($this->getView()->err = $validator->getErrors()) {
            ...
            if (!$this->advert->getTrack()->compare('category', $this->advert->getCategory())) {
                ...
                $bread_crumbs = new BreadCrumbs($path, '', '/');
                ...
                if ($user->getEmail()->getValue()) {
                    ...
                    $mail->host = Registry::getInstance()->HOSTINFO['HOST'];
                    $mail->send();
                }
            }
