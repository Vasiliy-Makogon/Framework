...
class FrontendCategoryList extends Controller
{
    public function run()
    {
        ...
        $this->getView()->getHelper('\Krugozor\Framework\Html\Title')->add($category->getName());
        ...
        $params = array(
            'where' => count($ids) > 1
                ? array('?f IN (?ai)' => array(Advert::getPropertyFieldName('category'), $ids))
                : array('?f = ?i' => array(Advert::getPropertyFieldName('category'), $ids[0])),
        ...
        if ($this->getView()->territories->country &&
            $region_name = $this->getRequest()->getRequest('region_name_en')) {
        ...
        if ($this->getView()->territories->country &&
            $this->getView()->territories->region &&
            $city_name = $this->getRequest()->getRequest('city_name_en')) {
        ...
        if ($this->getRequest()->getGet('type', 'string')) {
            ...
            $params['where']['AND `' . Advert::getPropertyFieldName('type') . '` = "?s"'] = array($this->getRequest()->getGet('type', 'string'));
        }
        ...
        $this->getView()->region_paths = new CoverArray();
        ...
        $this->getView()->getHelper('\Krugozor\Framework\Html\Title')->addPostfixInLastElement($this->getView()->bread_crumbs_postfix_text);
