...
abstract class Mapper
{
     ...
     * Krugozor\Framework\Module\User\Mapper\User             => user
     * Krugozor\Framework\Module\User\Mapper\City             => user-city
     * Krugozor\Framework\Module\SuperUser\Mapper\DefaultCity => super_user-default_city
     *
     ...
     */
    public function __construct(Manager $mapperManager)
    {
     ...
     * Например: Krugozor\Framework\Module\User\Model\User
     *
     * @return string
     */
    ...
    public function getTableName(): string
    {
        if (null === $this->db_table_name) {
            ...
            $module = Strings::camelCaseToProperty($this->module_name);
            $controller = Strings::camelCaseToProperty($this->model_name);
     ...
     * опций аттрибутов модели Model::model_attributes.
     *
     ...
     * @final
     */
    protected final function createModelFromDatabaseResult(array $data): Model
    {
     ...
     * @final
     */
    protected final function findModelBySql(): Model
    {
     ...
     * @return CoverArray
     */
    protected final function findModelListBySql(): CoverArray
    {
        if (!func_num_args() or func_num_args() && (!func_get_arg(0) || !is_string(func_get_arg(0)))) {
            return false;
        }
     ...
     */
    protected static function makeSqlFromParams(array $params): array
    {
        ...
        if (!empty($params['limit']) && is_array($params['limit'])) {
            $sql_store['limit'] = isset($params['limit']['start']) && Numeric::isDecimal($params['limit']['start'], true)
                ? ' LIMIT ' . $params['limit']['start'] .
                (isset($params['limit']['stop']) && Numeric::isDecimal($params['limit']['stop'], true)
                    ? ', ' . $params['limit']['stop']
                    : ''
                )
                : '';
     ...
     * @return array
     */
    protected function getTableMetada(): array
    {
    ...
    private final function getModelClassNameByParts(string $module, string $model): string
    {
        return 'Krugozor\\Framework\\Module\\' .
            ...
            '\\Model\\' . Strings::formatToCamelCaseStyle($model);
    }
